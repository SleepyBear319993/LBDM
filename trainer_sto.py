...
# --- Configuration ---
TRAIN_DATA_DIR = "lbm_sto_reversal_data_cifar0_32_train"  # Path to training HDF5 files
TEST_DATA_DIR = "lbm_sto_reversal_data_cifar0_32_test"    # Path to test/validation HDF5 files
IMG_SIZE = 32
N_CHANNELS = 27  # 3 RGB * 9 LBM distributions
N_OUT_CHANNELS = 27  # Output is noise_t with same shape
TOTAL_STEPS = 1000  # T used during data generation
BATCH_SIZE = 64
LEARNING_RATE = 1e-4
EPOCHS = 25
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
MODEL_SAVE_PATH = "unet_lbm_sto_model_32_reversal_epoch{epoch}.pth"
# --- ---

class LBMStochasticDiffusionDataset(Dataset):
    """
    Dataset for stochastic LBM diffusion reversal.
    Loads (f_t, noise_t) pairs instead of (f_t, delta_f).
    """
    ...
    def _check_data(self):
        ...
        # Open one file to check dimensions and verify it contains noise_t
        with h5py.File(self.file_paths[0], 'r') as hf:
            ...
            assert 'noise_t' in hf, f"'noise_t' dataset not found in {self.file_paths[0]}. Is this a stochastic reversal dataset?"
            assert hf['noise_t'].shape == hf['f_t'].shape, f"Shape mismatch between f_t and noise_t in {self.file_paths[0]}"
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(file_path, 'r') as hf:
            ...
            noise_t_data = hf['noise_t'][time_step_idx]  # Shape: (3, 9, ny, nx)

            # Combine channels and distributions: (3, 9, ny, nx) -> (27, ny, nx)
            f_t_combined = f_t_data.reshape(-1, f_t_data.shape[-2], f_t_data.shape[-1])
            noise_t_combined = noise_t_data.reshape(-1, noise_t_data.shape[-2], noise_t_data.shape[-1])
        ...
        return torch.from_numpy(f_t_combined).float(), time_step_t, torch.from_numpy(noise_t_combined).float()
...
def train():
    ...
    LOG_FILE_NAME = f"training_sto_{timestamp}.log"
    ...
    logging.info("="*60)
    logging.info("Training UNet for Stochastic LBM Diffusion Reversal")
    logging.info("Model will predict noise_t from f_t")
    logging.info("="*60)
    ...
    train_dataset = LBMStochasticDiffusionDataset(total_steps=TOTAL_STEPS, data_dir=TRAIN_DATA_DIR)
    ...
    if test_files:
        test_dataset = LBMStochasticDiffusionDataset(total_steps=TOTAL_STEPS, data_dir=TEST_DATA_DIR)
    ...
    logging.info(f"Model architecture: UNetLBM with {N_CHANNELS} input channels and {N_OUT_CHANNELS} output channels")

    # Loss and Optimizer
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
    logging.info(f"Optimizer: Adam with learning rate {LEARNING_RATE}")
    logging.info(f"Loss function: MSE Loss")

    logging.info("Starting training...")
    for epoch in range(EPOCHS):
        model.train()
        running_loss = 0.0
        for i, (f_t, t, noise_t_true) in enumerate(train_dataloader):
            f_t = f_t.to(DEVICE)
            t = t.to(DEVICE).squeeze(-1)
            noise_t_true = noise_t_true.to(DEVICE)

            optimizer.zero_grad()
            noise_t_pred = model(f_t, t)
            loss = criterion(noise_t_pred, noise_t_true)
            loss.backward()
            optimizer.step()
        ...
        if test_dataloader:
            ...
            with torch.no_grad():
                for i_test, (f_t_test, t_test, noise_t_true_test) in enumerate(test_dataloader):
                    f_t_test = f_t_test.to(DEVICE)
                    t_test = t_test.to(DEVICE).squeeze(-1)
                    noise_t_true_test = noise_t_true_test.to(DEVICE)

                    noise_t_pred_test = model(f_t_test, t_test)
                    test_loss_item = criterion(noise_t_pred_test, noise_t_true_test)
        ...
        if (epoch + 1) % 5 == 0:
            checkpoint_path = MODEL_SAVE_PATH.format(epoch=epoch+1)
