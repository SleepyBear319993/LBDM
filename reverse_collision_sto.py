from diffusion_sto import diffusion_collision_kernel_stochastic, diffusion_collision_kernel, idx, w_const
import numpy as np
...
def diffusion_collision_kernel_reverse_stochastic(fp, omega, noise_storage, step, nx, ny):
    ...
    if i < nx and j < ny:
        ...
        for k in range(9):
            ...
            fp[idx(i, j, k, nx, ny)] = (fp[idx(i, j, k, nx, ny)] - omega * feq - noise_term) / (1.0 - omega)
...
if __name__ == "__main__":
    ...
    # Forward stochastic diffusion with noise storage for each time step
    ...
    # Reverse stochastic diffusion using stored noise terms (in reverse order)
    for step in range(num_steps):
    ...
    elif relative_l2_error < 1e-6:
        print(f"\n✓ SUCCESS: Reverse stochastic diffusion is reasonably accurate (relative L2 error < 1e-6)")
    elif relative_l2_error < 1e-3:
        print(f"\n⚠ WARNING: Reverse stochastic diffusion has moderate accuracy (relative L2 error < 1e-3)")
    else:
        print(f"\n✗ FAILURE: Reverse stochastic diffusion has poor accuracy (relative L2 error >= 1e-3)")